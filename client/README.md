...
- **Go Live and chat** use [`parallax-server`](../server/). In Settings › Server, enter its URL and token, then connect **Twitch** and **YouTube** (you approve Parallax on their sites; no stream keys to copy). For YouTube, the Go Live sheet also asks for a title and visibility. **Go Live** encodes the program once (H.264 + AAC via [HaishinKit](https://github.com/HaishinKit/HaishinKit.swift)) and sends it to the server over SRT, reconnecting by itself if the connection drops; the server relays it to each platform. Chat from both shows up in the chat panel (YouTube's while you're live there), and replies post as you. Without a server, the **Mock** toggle in the chat header fills chat with fake messages; it is off by default.
- **Deploy**: Settings › Server › Deploy creates a DigitalOcean droplet running `parallax-server` (HTTPS via Caddy, a firewall, an encrypted SRT upload) and fills in the URL and token when it's up. It also lists, updates, and destroys those servers. See the [DigitalOcean setup](../docs/setup/digitalocean.md).
